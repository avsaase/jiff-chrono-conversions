...
impl ToChrono<chrono::DateTime<chrono::Utc>> for jiff::Zoned {
    ...
    ///
    /// This conversion is infallible: unlike `chrono::FixedOffset` or `chrono_tz::Tz`,
    /// `chrono::Utc` carries no time zone identity of its own, so any instant can always be
    /// expressed in it.

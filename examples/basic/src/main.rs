...
fn main() {
    ...
    let jiff_datetime: jiff::civil::DateTime = chrono_datetime.to_jiff().unwrap();
    ...
    let jiff_timestamp: jiff::Timestamp = chrono_utc_datetime.to_jiff().unwrap();
    ...
    let chrono_offset: chrono::FixedOffset = jiff_offset.to_chrono().unwrap();
    ...
    let chrono_fixed_datetime: chrono::DateTime<chrono::FixedOffset> = jiff_zoned.to_chrono().unwrap();
    println!("zoned datetime: jiff {jiff_zoned} -> chrono {chrono_fixed_datetime}");
    let jiff_zoned: jiff::Zoned = chrono_fixed_datetime.to_jiff().unwrap();
    ...
    let jiff_tz: jiff::tz::TimeZone = chrono_tz.to_jiff().unwrap();
    println!("time zone: chrono {chrono_tz} -> jiff {}", jiff_tz.iana_name().unwrap());
    let chrono_tz: chrono_tz::Tz = jiff_tz.to_chrono().unwrap();
